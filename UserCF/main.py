...
#        f.write(json.dumps(similarity))
    ...
    target_rating = rating[rating.user_id == target_user]
    targetAnime = set(target_rating[target_rating.rating > 5].anime_id.tolist())
    likeAnime = set(likeAnime)
    reclist = set(likeAnime) & (targetAnime ^ set(likeAnime))


    recall = len(likeAnime & targetAnime) / len(likeAnime)
    precise = len(likeAnime & targetAnime) / len(targetAnime)
